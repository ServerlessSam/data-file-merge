...
class SourceFile():
    ...
    def retrieve_content(self):
        ...
        for src_file in self.source_file_location.resolved_paths:
            src_content = JsonFileType.load_from_file(
                src_file
            )
            jsonpath_expr = parse(self.source_file_root)
            retrieved_src_content.extend([match.value for match in jsonpath_expr.find(src_content)])
...
class DestinationFile():
    ...
    def __post_init__(self):
        if len(self.destination_file_location.resolved_paths) > 1:
            raise Exception("Attempting to use multiple destination files. We don't support this (yet)!")
...
class BuildConfig():
    ...
    def __eq__(self, other):
        ...
        for src, other_src in zip(self.source_files, other.source_files):
            if src.source_file_root != other_src.source_file_root:
                ...
                break
            if src.source_file_location.substituted_path != other_src.source_file_location.substituted_path:
                src_files_match=False
                break
        ...
        return self.destination_file.destination_file_location.substituted_path == other.destination_file.destination_file_location.substituted_path and \
            self.destination_file.file_content == other.destination_file.file_content and \
            src_files_match
    '''
    ...
    def load_config_from_file(file_path:Path, parameters:dict={}):
        ...
        for src in config_dict["SourceFiles"]:
            ...
            for sub_key, sub_dict in src["SourceFileLocation"]["PathSubs"].items():
                ...
                subs[sub_key] = Substitution(
                    reference_type(
                        parameters,
                        None # TODO support reading from content
                    ),
                    sub_dict["Value"],
                    regex
                )
        ...
        dest_subs = {}
        for sub_key, sub_dict in config_dict["DestinationFile"]["DestinationFileLocation"]["PathSubs"].items():
            reference_type = ReferenceTypeFactory(sub_dict["Type"]).generate()
            regex = sub_dict["Regex"] if "Regex" in sub_dict else None
