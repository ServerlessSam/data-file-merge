...
class BaseNamingConvention():
    ...
    def convert_to_list(self, string_to_convert:str) -> list[str]:
        ...
        if not strings_found:
            raise Exception(f"{string_to_convert} is not of the expected naming convention.")
...
class PascalCase(BaseNamingConvention):
    ...
    def convert_from_list(self, list_to_convert: list[str]) -> str:
        ...
        for word in list_to_convert:
            if not word.islower():
                raise Exception("String wasn't lowercase for some reason. Something has gone wrong.")
            word[0] = word[0].upper()
            to_return+=word
        return to_return
...
class SnakeCase(BaseNamingConvention):
    ...
    def convert_to_list(self, string_to_convert:str) -> list[str]:
        ...
        if len("".join( str(found) for found in strings_found)) != len(string_to_convert) - len(strings_found) + 1:
...
class ConversionStringParser():
    ...
    def parse(self):
        ...
        if len(split_string) != 2:
            raise Exception(f"Conversion string {self.conversion_string} is not of the expected format.")
        for individual_string in split_string:
            if individual_string not in self.SUPPORTED_CONVENTIONS:
                raise Exception(f"Conversion string requires {individual_string}, but that is not a supported/recognised casing.")
...
class StringConverter():
    ...
    def convert(self, string_to_convert:str) -> str:
        return self.to_convention.convert_from_list(
            self.from_convention.convert_to_list(
                string_to_convert
            )
        )