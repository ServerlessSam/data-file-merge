...
class ContentReferenceType(BaseReferenceType):
    ...
    def evaluate(self, value:str, regex:RegexExtractor=None) -> str:
        ...
        jsonpath_matches = [match.value for match in jsonpath_expr.find(self.file_content)]
        if len(jsonpath_matches) != 1:
            raise Exception(f"Content reference type returned {len(jsonpath_matches)} matches instead of the required 1.")
        value_in_file = jsonpath_matches[0]
        if type(value_in_file) not in [int, str]:
            raise Exception(f"Content from file: {str(value_in_file)} is not a string or int.")
        if regex:
...
class KeyReferenceType(BaseReferenceType):
    ...
    def evaluate(self, value:str, regex:str=None) -> str:
        ...
        jsonpath_matches = [match.value for match in jsonpath_expr.find(self.file_content)]
        if len(jsonpath_matches) != 1:
            raise Exception(f"Key reference type returned {len(jsonpath_matches)} matches instead of the required 1. A key reference type must return a dictionary with a single key.")
...
class ReferenceTypeFactory:
    ...
    REFERENCE_TYPE_MAPPING = tuple(zip(
        [
            "Parameter",
            "Content",
            "Key",
    ...
    def generate(self):
        for type in self.REFERENCE_TYPE_MAPPING:
            if self.reference_type_str == type[0]:
                return type[1]
        raise Exception(f"Reference type was not found for string: {self.reference_type_str}")